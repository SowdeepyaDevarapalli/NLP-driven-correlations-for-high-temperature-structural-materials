...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        # Support both legacy PyTorch binary and new safetensors formats
        if os.path.isdir(model_path):
            bin_path = os.path.join(model_path, 'pytorch_model.bin')
            safe_path = os.path.join(model_path, 'model.safetensors')
        else:
            bin_path = model_path if model_path.endswith('.bin') else None
            safe_path = model_path if model_path.endswith('.safetensors') else None

        state_dict = None
        if bin_path and os.path.exists(bin_path):
            state_dict = torch.load(bin_path, map_location=torch.device('cpu'))
        elif safe_path and os.path.exists(safe_path):
            try:
                from safetensors.torch import load_file as safe_load
                state_dict = safe_load(safe_path, device='cpu')
            except Exception as e:
                # Can't read safetensors (package missing or error) -> skip version check
                print(f"[WARN] Skipping version check: could not read safetensors ({e}).")
                return
        else:
            # No recognizable weight file -> skip version check
            print("[WARN] Skipping version check: no pytorch_model.bin or model.safetensors found.")
            return
