...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        # parse tokens with truncation to fit BERT's max position embeddings (typically 512)
        doc_tokens, doc_encoding = _parse_tokens_truncated(jtokens, dataset, self._tokenizer)

        # parse entity mentions, filtering those that exceed truncated token count
        entities, ent_idx_map = self._parse_entities(jentities, doc_tokens, dataset, return_index_map=True)

        # parse relations, dropping those that reference filtered entities
        relations = self._parse_relations(jrelations, entities, dataset, ent_idx_map=ent_idx_map)
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset, return_index_map: bool = False):
        ...
        for entity_idx, jentity in enumerate(jentities):
            ...
            # skip entities that fall outside truncated token range
            if end > token_count:
                idx_map[entity_idx] = None
                continue
            ...
            idx_map[entity_idx] = len(entities)
            entities.append(entity)

        if return_index_map:
            return entities, idx_map
        return entities

    def _parse_relations(self, jrelations, entities, dataset, ent_idx_map=None) -> List[Relation]:
        relations = []

        for jrelation in jrelations:
            relation_type = self._relation_types[jrelation['type']]

            # map original entity indices to filtered indices (if provided)
            if ent_idx_map is not None:
                head_idx = ent_idx_map.get(jrelation['head'])
                tail_idx = ent_idx_map.get(jrelation['tail'])
                # drop relations that involve filtered-out entities
...
def _parse_tokens(jtokens, dataset, tokenizer):
    """Legacy non-truncating tokenization (kept for compatibility)."""
    max_pos = 512
    return _parse_tokens_truncated(jtokens, dataset, tokenizer, max_position_embeddings=max_pos)


def _parse_tokens_truncated(jtokens, dataset, tokenizer, max_position_embeddings: int = 512):
    """Tokenize and build encoding truncated to max_position_embeddings (incl. special tokens)."""
    doc_tokens = []

    # full document encoding including special tokens ([CLS] and [SEP]) and BPE encodings of original tokens
    doc_encoding = [tokenizer.convert_tokens_to_ids('[CLS]')]
    # reserve space for final [SEP]
    max_len_without_sep = max(1, max_position_embeddings - 1)

    # parse tokens up to budget
    for i, token_phrase in enumerate(jtokens):
        ...
        # stop if adding this token would exceed budget (excluding final [SEP])
        if len(doc_encoding) + len(token_encoding) > max_len_without_sep:
            break
