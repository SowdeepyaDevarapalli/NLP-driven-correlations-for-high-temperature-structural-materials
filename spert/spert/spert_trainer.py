...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        # AdamW from transformers supports 'correct_bias', torch.optim.AdamW does not.
        # Try with 'correct_bias' first, and on TypeError fall back without it.
        try:
        ...
        if args.init_eval:
            try:
                ...
                self._logger.warning(f"Initial eval failed: {e}. Continuing training.")

        # train with robust error handling
        completed_epochs = 0
        try:
            for epoch in range(args.epochs):
                self._logger.info(f"Starting epoch {epoch+1}/{args.epochs}")
                try:
                    # train epoch
                    self._train_epoch(model, compute_loss, optimizer, train_dataset, updates_epoch, epoch)
                    completed_epochs = epoch + 1
                    self._logger.info(f"Completed training epoch {completed_epochs}")
                    ...
                    if not args.final_eval or (epoch == args.epochs - 1):
                        try:
                            ...
                            self._logger.warning(f"Eval failed at epoch {epoch+1}: {e}. Skipping eval and continuing.")
                except Exception as e:
                    self._logger.error(f"Training failed at epoch {epoch+1}: {e}. Stopping training.")
                    break
        except Exception as e:
            self._logger.error(f"Training loop failed: {e}")

        # always save final model, even if training terminated early
        try:
            extra = dict(epoch=completed_epochs, updates_epoch=updates_epoch, epoch_iteration=0)
            global_iteration = completed_epochs * updates_epoch
            self._save_model(self._save_path, model, self._tokenizer, global_iteration,
                             optimizer=optimizer if self._args.save_optimizer else None, extra=extra,
                             include_iteration=False, name='final_model')
            self._logger.info(f"Final model saved after {completed_epochs} epochs")
        except Exception as e:
            self._logger.error(f"Failed to save final model: {e}")
